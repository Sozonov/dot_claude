Отлично, мы обсудили все детали. Теперь твоя задача — финализировать наш план.
...
2.  **Определи папку для файлов плана:**
    ...
    - Если не использовали файл задачи, то спроси у пользователя номер задачи. И по ней сгенерируй путь типа anton/tasks/PS-8572
...
6.  **Заполни файлы начальным содержимым:**
    ...
    - В шаге финализации убедись, что в подзадачи ты добавил все проверки из anton/docs/linters.md
...
1.  Выполни проверки согласно инструкции anton/docs/linters.md. ВАЖНО! Выполняй все команды из это linters.md! Исправляй проблемы, пока все проверки не пройдут. 
...
1.  **Создать и сохранить** все артефакты протокола (`plan.md`, `context.md`, `log.md`, `00-setup.md` и файлы для всех последующих шагов) в `anton/tasks/PS-NNNN`.
...
2.  **Верификация:** **После завершения ВСЕХ подзадач** запусти полный цикл проверок для измененных файлов по инструкции anton/docs/linters.md
